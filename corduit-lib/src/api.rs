...
fn init_tracing_safe() -> std::result::Result<(), ()> {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new(
            "debug,corduit_core=debug,corduit_lib=debug,corduit_netstack=debug,hyper=warn,tokio=warn,rustls=warn"
        ));

    let bridge_log_layer = BridgeLogLayer;
    #[cfg(target_os = "android")]
    {
        let android_layer = tracing_android::layer("Corduit").map_err(|error| {
    ...
    #[cfg(not(target_os = "android"))]
    {
        ...
        if result.is_ok() {
            tracing::info!("Corduit FFI bridge initialized");
        }
        result.map_err(|_| ())
    }
}
...
pub async fn start_proxy_from_yaml(yaml_config: String) -> std::result::Result<(), String> {
    ...
    let corduit = corduit_core::Corduit::new(config)
        .await
...
pub async fn stop_proxy() -> std::result::Result<(), String> {
    ...
    let tracker = corduit_core::connection_tracker::global_tracker();
    tracker.reset();
    {
    ...
    if let Some(corduit) = guard.as_ref() {
        ...
        for outbound in &config.outbounds {
            let group_type = match outbound.outbound_type {
                corduit_core::OutboundType::Selector => "selector",
                corduit_core::OutboundType::Urltest => "url-test",
                corduit_core::OutboundType::Fallback => "fallback",
                corduit_core::OutboundType::Loadbalance => "load-balance",
                corduit_core::OutboundType::Relay => "relay",
                _ => continue,
            };
...
pub async fn set_proxy_mode(mode: i32) -> std::result::Result<(), String> {
    corduit_core::set_runtime_proxy_mode(mode);

    #[cfg(target_os = "android")]
    {
        corduit_netstack::set_android_proxy_mode(mode);
    }

    #[cfg(windows)]
    {
        corduit_netstack::set_windows_proxy_mode(mode);
    }
...
pub async fn get_proxy_mode() -> std::result::Result<i32, String> {
    Ok(corduit_core::get_runtime_proxy_mode())
}
...
pub async fn start_tun_mode(
    ...
    #[cfg(windows)]
    {
        use corduit_netstack::{TunConfig, TunDevice};
        tracing::info!(
        ...
        corduit_netstack::ensure_wintun()
            .await
...
pub async fn stop_tun_mode() -> std::result::Result<(), String> {
    #[cfg(windows)]
    {
        ...
        corduit_core::set_runtime_proxy_mode(0);
        corduit_netstack::set_windows_proxy_mode(0);
...
pub async fn initialize_corduit(config_json: String) -> Result<()> {
    ...
    let corduit = corduit_core::Corduit::new(core_config)
        .await
...
pub async fn test_config(config_json: String) -> Result<bool> {
    ...
    let core_config = convert_ffi_config_to_core(config)?;
    match corduit_core::Corduit::new(core_config).await {
        Ok(_) => Ok(true),
        Err(_) => Ok(false),
    }
}
...
pub async fn get_logs(lines: Option<u32>) -> Result<Vec<String>> {
    ...
    let logs = corduit_core::logging::get_recent_logs(max_lines);
    if logs.is_empty() {
        Ok(vec![
            "[INFO] Corduit log buffer initialized. Logs will appear here.".to_string(),
        ])
    } else {
        Ok(logs)
    }
...
pub fn get_version() -> String {
    format!("Corduit v{}", env!("CARGO_PKG_VERSION"))
}
...
fn configure_rule_providers(config_json: &str) -> Result<()> {
    use corduit_core::rule_provider::{RuleProviderConfig, RuleProviderType};
    ...
    let providers: Vec<RuleProviderConfig> = nextjson_to_core(&providers)
        .map_err(|error| CorduitError::Parse(format!("Invalid rule providers: {error}")))?;
    ...
    corduit_core::set_runtime_rule_providers(providers);
    Ok(())
}
...
fn parse_inbound_type(value: &str, tag: &str) -> Result<corduit_core::InboundType> {
    use corduit_core::InboundType;
...
fn parse_outbound_type(value: &str, tag: &str) -> Result<corduit_core::OutboundType> {
    use corduit_core::OutboundType;
...
fn parse_rule_type(value: &str) -> Result<corduit_core::RuleType> {
    use corduit_core::RuleType;
                ...
                if let Some(corduit) = guard.as_ref() {
                    ...
                    for inbound in &config.inbounds {
                        if matches!(
                            ...
                            corduit_core::InboundType::Mixed | corduit_core::InboundType::Http
                        ) {
                            port = inbound.port;
                            break;
                        }
...
pub fn is_wintun_available() -> bool {
    corduit_netstack::check_wintun_available()
}

pub fn get_wintun_dll_path() -> Option<String> {
    corduit_netstack::get_wintun_path().map(|p| p.to_string_lossy().to_string())
}

pub async fn ensure_wintun_dll() -> Result<String> {
    let path = corduit_netstack::ensure_wintun()
        .await
...
pub async fn enable_tun_mode_with_mode(mode: String) -> Result<TunStatus> {
    ...
    {
        use corduit_netstack::{TunConfig, TunDevice, WindowsRouteManager, WindowsVpnProcessor};
        ...
        if !cleanup_errors.is_empty() {
            return Ok(TunStatus {
                enabled: true,
                interface_name: Some("Corduit".to_string()),
                mtu: Some(1500),
                error: Some(format!(
        ...
        match corduit_netstack::ensure_wintun().await {
            Ok(path) => {
            ...
            if let Some(ref corduit) = *corduit_guard {
                ...
                if !is_running {
                    ...
                    return Ok(TunStatus {
                        ...
                        mtu: None,
                        error: Some(
                            "Corduit proxy service is not running. Please start it first."
                                .to_string(),
                        ),
                    });
                }
                ...
                return Ok(TunStatus {
                    ...
                    interface_name: None,
                    mtu: None,
                    error: Some("Corduit not initialized".to_string()),
                });
            }
        };
        ...
        if let Err(e) = corduit_netstack::set_tun_dns(&config.name, &config.dns) {
            tracing::warn!("Failed to set TUN DNS: {}", e);
        } else {
            tracing::info!("TUN DNS set to {:?}", config.dns);
        }

        if let Err(e) = corduit_netstack::flush_dns_cache() {
        ...
        crate::set_windows_tun_device(tun);
        corduit_core::set_runtime_proxy_mode(mode_int);
        corduit_netstack::set_windows_proxy_mode(mode_int);
    ...
    #[cfg(target_os = "linux")]
    {
        use corduit_netstack::{RouteManager, TunConfig, TunDevice, TunPacketProcessor};
        ...
        let (proxy_addr, proxy_servers) = {
            ...
            if !corduit.is_running().await.unwrap_or(false) {
                return Ok(TunStatus {
                    ...
                    interface_name: None,
                    mtu: None,
                    error: Some("Corduit proxy service is not running".to_string()),
                });
            }
            let config = corduit.config();
            let proxy_port = config
    ...
    {
        corduit_core::set_runtime_proxy_mode(mode_int);
        corduit_netstack::set_android_proxy_mode(mode_int);
        let enabled = start_android_vpn_inner().await?;
        if !enabled {
            corduit_core::set_runtime_proxy_mode(0);
            corduit_netstack::set_android_proxy_mode(0);
        }
        Ok(TunStatus {
            enabled,
...
pub async fn get_tun_status() -> Result<TunStatus> {
    ...
    #[cfg(target_os = "android")]
    {
        if corduit_netstack::get_android_vpn_fd() >= 0 {
            if let Some(processor) = crate::get_android_vpn_processor() {
                if processor.is_running() {
                    return Ok(TunStatus {
                        enabled: true,
    ...
    #[cfg(target_os = "linux")]
    {
            ...
            && crate::linux_packet_task_is_running()
        {
            return Ok(TunStatus {
                enabled: true,
                interface_name: Some("corduit0".to_string()),
                mtu: Some(1500),
                error: None,
            });
        }
...
pub fn get_windows_proxy_mode_str() -> String {
    match corduit_core::get_runtime_proxy_mode() {
        1 => "global".to_string(),
        2 => "direct".to_string(),
        3 => "rule".to_string(),
        _ => "rule".to_string(),
...
pub fn get_android_vpn_fd() -> i32 {
    #[cfg(target_os = "android")]
    {
        corduit_netstack::get_android_vpn_fd()
    }
...
async fn start_android_vpn_inner() -> Result<bool> {
    #[cfg(target_os = "android")]
    {
        ...
        let fd = corduit_netstack::get_android_vpn_fd();
        if fd < 0 {
        ...
        if corduit_netstack::has_protect_callback() {
        ...
        let (proxy_addr, proxy_port) = {
            ...
            if let Some(ref corduit) = *corduit_guard {
                ...
                if !is_running {
                    ...
                    tracing::error!(
                        "Please start Corduit proxy service first before enabling VPN."
                    );
                    return Ok(false);
                }
                tracing::info!("Corduit proxy service is running");
        ...
        match tokio::net::TcpStream::connect(proxy_addr).await {
            ...
            Err(e) => {
                ...
                tracing::error!("Corduit proxy may not have started correctly");
                return Ok(false);
            }
        }
...
mod config_conversion_tests {
    ...
    fn supported_aliases_are_explicit() {
        assert_eq!(
            ...
            corduit_core::OutboundType::Hysteria2
        );
        assert_eq!(
            parse_outbound_type("socks", "proxy").unwrap(),
            corduit_core::OutboundType::Socks5
        );
    }
    ...
    fn unsupported_protocols_fail_closed() {
        for protocol in ["hysteria", "naive", "typo"] {
            ...
            assert!(matches!(error, CorduitError::Config(_)));
