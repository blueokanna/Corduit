...
公开 DNS 对这些域名要么 NXDOMAIN（解析瞬间失败，连接还没建立就被放弃——表现为零点几秒的"静默断开"），要么返回诱饵地址（连过去必然黑洞）。0.1.6 起引擎解析**自己的出站服务器域名**时：先查 `nameserver-policy`（按最长后缀命中，`+.`/`*.`/裸域名写法都支持），命中就只问策略里的解析器；其余名字走 `nameservers`；系统解析器仅在引擎没有任何可用 DNS 配置时兜底。结果会做 5 分钟正向缓存。回归测试：`dns::engine_resolver` 的 5 个单测（域名后缀归一化、标签边界匹配、最长策略优先、环回 UDP 解析器实测 + 缓存命中）。
...
**Q: 日志出现 `engine DNS found no address for '<域名>'`，但同一台机器上系统解析同一名字没问题？**

这是一次解析被"半死不活的上游"拖垮的典型组合，0.2.3 起两道缺口都补上了：

1. **上游失败不再一票否决（recurse-x 0.2.2）**：`Forwarders::exchange_with` 以前拿到第一个与问题匹配的响应就返回，不看 RCODE——某个 nameserver 回 SERVFAIL/REFUSED（订阅里的 DNS 列表恰好有一个不通的成员是常态）会让整次解析失败，哪怕其余六个上游都答得好好的。现在失败响应只回填 `record_servfail` 并继续问下一个上游；`NXDOMAIN` 仍是终止答案（名字不存在就是答案，不必再问别人）。全部失败时返回最后一个失败（`REFUSED` 以 `ErrorKind::Refused` 上抛）及其端点，不再以"空答案"收场。
2. **引擎解析空手而归时落回系统解析器（本版本）**：TUN/透明代理路径的 `resolve_real` 此前在引擎解析失败或返回空时直接报错，而**出站路径**（引擎解析自己的服务器域名）同样的错误早有系统兜底——实现与自己文档的承诺不一致。现在两条路径同一条策略：引擎解析拿到非空答案就直接用；返回空或报错则用系统解析器兜底——"引擎 DNS 配置不可用但系统还能解析"正是它救场的场景；两边都失败时错误信息同时给出两个原因（`engine DNS: <原因>; the system resolver failed too: <原因>`），不再让用户看到一句无从下手的 "found no address"。

**Q: 节点在别的客户端里能用（延迟测试正常），在引擎里却"秒连秒断"，日志同样没有任何错误？**

这是 **legacy（pre-AEAD）VMess 服务器**的典型特征。订阅里的 `alterId > 0` 不是历史包袱——它意味着服务器（多为老版本 v2ray 或兼容配置）只接受**旧版握手**：客户端发送 `HMAC-MD5` 鉴权块 + AES-128-CFB 封装的请求头，服务器才认；而纯 AEAD 握手（现代客户端默认）会被服务器**直接关闭连接**，不回任何错误——表现就是零点几秒的静默断开，连参考实现 Xray 的最新版（同样只发 AEAD）打不通这类节点，只有按 `alterId>0` 发 legacy 握手的客户端能连。

引擎的行为是：**`alterId > 0` 自动选择 legacy 握手**，`alterId = 0` 走 AEAD：
