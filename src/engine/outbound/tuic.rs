//! TUIC v5 outbound on the in-repo QUIC transport.
//!
//! Wire protocol (TUIC v5, as implemented by `tuic` servers):
//!
//! ```text
//! authenticate  uni-stream [VER=0x05][CMD=0x00][UUID 16][TOKEN 32]
//!   TOKEN       = SHA256(UUID bytes || password)
//! connect       bi-stream  [VER=0x05][CMD=0x01][ADDR]
//! packet        datagram   [VER=0x05][CMD=0x02][assoc_id u16][frag_total u8]
...
//! datagrams). Congestion-control selection maps onto the transport's
...
/// TUIC outbound settings (kept for `tuic_config()` introspection and tests).
#[derive(Debug, Clone)]
pub struct TuicConfig {
    ...
    pub congestion_control: crate::protocol::quic::CongestionControl,
...
impl Default for TuicConfig {
    fn default() -> Self {
        Self {
            ...
            disable_sni: false,
            congestion_control: crate::protocol::quic::CongestionControl::default(),
...
/// A live, authenticated TUIC v5 session over one QUIC connection.
...
impl TuicConnection {
    ...
    pub async fn send_udp_packet(
        ...
        debug!("TUIC UDP packet sent to {target} ({} bytes)", data.len());
        Ok(())
    }
...
/// TUIC token: `SHA256(UUID bytes || password)` (32 bytes).
...
impl TuicOutbound {
    pub fn new(config: OutboundConfig) -> Result<Self> {
        ...
        let congestion_control = config
            ...
            .map(yaml_value_to_string)
            .and_then(|s| crate::protocol::quic::CongestionControl::parse_str(&s))
            .unwrap_or_default();
        ...
        if zero_rtt_handshake {
            warn!(
                "TUIC '{}': zero-rtt-handshake / reduce-rtt is not supported by the \
                 in-repo QUIC transport; continuing with a 1-RTT handshake",
                config.tag
            );
        }
    ...
    fn build_quic_config(&self, socket_addr: SocketAddr) -> Result<QuicClientConfig> {
        ...
        cfg.congestion_control = self.tuic_config.congestion_control;
        Ok(cfg)
    }
...
mod tests {
    ...
    #[test]
    fn test_udp_relay_mode_default() {
        assert_eq!(UdpRelayMode::default(), UdpRelayMode::Native);
        assert!("native".parse::<UdpRelayMode>().unwrap() == UdpRelayMode::Native);
        assert!("quic".parse::<UdpRelayMode>().unwrap() == UdpRelayMode::Quic);
        assert!("bogus".parse::<UdpRelayMode>().is_err());
    }
    ...
    fn test_tuic_config_default() {
        ...
        assert_eq!(config.heartbeat, 10000);
    }

    #[test]
    fn test_auth_token_known_vector() {
        // SHA256(16 zero bytes || "password") — cross-checked independently.
    ...
    fn test_encode_address_ipv6() {
        ...
        assert_eq!(
            &buf[1..17],
            std::net::Ipv6Addr::LOCALHOST.octets().as_slice()
        );
    ...
    fn test_parse_address_roundtrip() {
        ...
        for target in cases {
            let mut buf = BytesMut::new();
            encode_address(&mut buf, &target).unwrap();
            let (decoded, rest) = parse_address(&buf).unwrap();
    ...
    fn test_parse_address_rejects_garbage() {
        ...
        assert!(parse_address(&[TUIC_ADDR_TYPE_IPV4, 1, 2, 3]).is_err());
        assert!(parse_address(&[TUIC_ADDR_TYPE_DOMAIN, 5, b'a']).is_err());
        assert!(parse_address(&[0x99]).is_err());
    }
    ...
    #[test]
    fn test_parse_udp_packet_rejects_truncated() {
        assert!(parse_udp_packet(&[0u8; 7]).is_err());
