...
impl WireguardOutbound {
    ...
    pub async fn relay_udp(&self, target: &TargetAddr, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let src_port = 40000 + (crate::engine::random::u16() % 20000);
        ...
        packet[4..6].copy_from_slice(&crate::engine::random::u16().to_be_bytes());
        packet[6] = 0x40;
        packet[7] = 0x00;
        packet[8] = 64;
        packet[9] = 17;
...
mod tests {
    use super::*;
    use crate::protocol::wireguard::generate_keypair;
    ...
    fn test_wireguard_outbound_new() {
        ...
        let config = OutboundConfig {
            ...
            outbound_type: crate::engine::config::OutboundType::Wireguard,
