...
fn init_tracing_safe() -> std::result::Result<(), ()> {
    ...
    #[cfg(target_os = "android")]
    {
        let android_layer = tracing_android::layer("Corduit").map_err(|error| {
            crate::engine::logging::add_log(format!(
...
pub async fn set_proxy_mode(mode: i32) -> std::result::Result<(), String> {
    ...
    #[cfg(target_os = "android")]
    {
        crate::netstack::set_android_proxy_mode(mode);
    }

    #[cfg(windows)]
    {
        crate::netstack::set_windows_proxy_mode(mode);
    }
...
pub fn set_vpn_fd(fd: i32) {
    ...
    {
        #[cfg(target_os = "android")]
        crate::netstack::set_android_vpn_fd(fd);
        #[cfg(target_os = "ios")]
        crate::netstack::set_ios_vpn_fd(fd);
...
pub fn clear_vpn_fd() {
    ...
    {
        #[cfg(target_os = "android")]
        crate::netstack::clear_android_vpn_fd();
        #[cfg(target_os = "ios")]
        crate::netstack::clear_ios_vpn_fd();
...
/// Handle of the running localhost JSON-RPC server, if any.
static RPC_SERVER: once_cell::sync::Lazy<
    parking_lot::Mutex<Option<crate::rpc::server::RpcServerHandle>>,
...
/// Stop the localhost JSON-RPC server if it is running.
pub fn stop_rpc_server() -> std::result::Result<(), String> {
    let mut guard = RPC_SERVER.lock();
    if let Some(handle) = guard.take() {
        handle.stop();
        tracing::info!("RPC server stopped");
    } else {
        tracing::debug!("RPC server was not running");
    }
    Ok(())
}

/// Report whether the localhost JSON-RPC server is running and its address.
pub fn get_rpc_server_status() -> std::result::Result<RpcServerStatus, String> {
    let guard = RPC_SERVER.lock();
    match guard.as_ref() {
        Some(handle) if handle.is_running() => Ok(RpcServerStatus {
            running: true,
            addr: Some(handle.addr().to_string()),
            token_set: handle.token_set(),
        }),
        _ => Ok(RpcServerStatus {
            running: false,
            addr: None,
            token_set: false,
        }),
...
fn configure_rule_providers(config_json: &str) -> Result<()> {
    use crate::engine::rule_provider::{RuleProviderConfig, RuleProviderType};
...
fn parse_inbound_type(value: &str, tag: &str) -> Result<crate::engine::InboundType> {
    use crate::engine::InboundType;
...
fn parse_outbound_type(value: &str, tag: &str) -> Result<crate::engine::OutboundType> {
    use crate::engine::OutboundType;
...
fn parse_rule_type(value: &str) -> Result<crate::engine::RuleType> {
    use crate::engine::RuleType;
...
pub async fn test_shadowsocks_latency(
    ...
    let timeout_duration = Duration::from_millis(timeout_ms as u64);
    let test_url = "http://www.gstatic.com/generate_204";
    let url = match crate::common::url::Url::parse(test_url) {
        Ok(u) => u,
        Err(e) => {
            return Ok(LatencyTestResult {
                proxy_name,
...
enum SsAeadCipherInner {
    Aes256Gcm(crate::crypto::aead::Aes256Gcm),
    Aes128Gcm(crate::crypto::aead::Aes128Gcm),
}
    ...
    fn new(key: &[u8]) -> std::result::Result<Self, String> {
        let inner = if key.len() == 32 {
            SsAeadCipherInner::Aes256Gcm(
                crate::crypto::aead::Aes256Gcm::new_from_slice(key)
                    .map_err(|_| "Invalid AES-256 key")?,
            )
        } else if key.len() == 16 {
            SsAeadCipherInner::Aes128Gcm(
                crate::crypto::aead::Aes128Gcm::new_from_slice(key)
...
pub fn is_wintun_available() -> bool {
    crate::netstack::check_wintun_available()
}
...
pub async fn enable_tun_mode_with_mode(mode: String) -> Result<TunStatus> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Err(e) = crate::netstack::set_tun_dns(&config.name, &config.dns) {
            tracing::warn!("Failed to set TUN DNS: {}", e);
        } else {
            tracing::info!("TUN DNS set to {:?}", config.dns);
        }

        if let Err(e) = crate::netstack::flush_dns_cache() {
        ...
        crate::netstack::set_windows_proxy_mode(mode_int);
...
pub async fn get_tun_status() -> Result<TunStatus> {
    ...
    #[cfg(target_os = "android")]
    {
        if crate::netstack::get_android_vpn_fd() >= 0 {
            if let Some(processor) = crate::get_android_vpn_processor() {
                if processor.is_running() {
                    return Ok(TunStatus {
                        enabled: true,
...
pub fn get_windows_proxy_mode_str() -> String {
    match crate::engine::get_runtime_proxy_mode() {
        1 => "global".to_string(),
        2 => "direct".to_string(),
        3 => "rule".to_string(),
        _ => "rule".to_string(),
...
pub fn get_android_vpn_fd() -> i32 {
    #[cfg(target_os = "android")]
    {
        crate::netstack::get_android_vpn_fd()
    }
...
async fn start_android_vpn_inner() -> Result<bool> {
    #[cfg(target_os = "android")]
    {
        ...
        let fd = crate::netstack::get_android_vpn_fd();
        if fd < 0 {
        ...
        if crate::netstack::has_protect_callback() {
...
mod config_conversion_tests {
    ...
    fn local_proxy_endpoint_follows_inbound_address_family() {
        use crate::engine::{InboundConfig, InboundType};
        use std::net::{IpAddr, Ipv4Addr, Ipv6Addr, SocketAddr};
